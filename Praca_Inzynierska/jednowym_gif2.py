...
lista_obrazow = []
lista_RMSE = []
rozmiar_tablicy = 101                                # tutaj ustawiamy rozmiar tablicy

oczekiwany_wynik = [(100-i) for i in range(101)]
rzeczywisty_wynik = [0 for i in range(101)]
...
def krok(tab):
    ...
    for i in range(len(tab)):
        if(i > 0 and i < (len(tab) - 1)):
            ...
            tab[i].temp = round((tab[i-1].temp_poprzednia + tab[i].temp_poprzednia + tab[i+1].temp_poprzednia)/3, 10)
...
if __name__ == '__main__':
    ...
    tab[0].temp = 100                                    # tutaj ustawiamy punkty grzania
    tab[0].temp_poprzednia = 100
